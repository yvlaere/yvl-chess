...
/*
    std::string fen1 = "rnbqkbnr/pppppppp/8/8/8/5P2/PPPPP1PP/RNBQKBNR b KQkq - 0 1";
    bool color = true;
    state = fen_to_game_state(fen1, color);
*/
    ...
    game_state_to_input(piece_on_square, active_features_w, active_features_b, color);

    for (int i = 0; i < active_features_w.size(); i++) {
        std::cout << active_features_w[i] << " ";
    }
    std::cout << std::endl;
    std::cout << "----------------" << std::endl;
    for (int i = 0; i < active_features_b.size(); i++) {
        std::cout << active_features_b[i] << " ";
    }
